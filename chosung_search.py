...
def chosung_indexing(cho_indexing, filename):
    ...
    with open (filename) as fin:
        ...
        for line in fin.readlines():                
           ...
           segments = line.split('\t')
           w = segments[0].rstrip()
           freq = int(segments[1].rstrip())
           num = (w, freq)
           chosung = str2chostr(w)
           if chosung not in cho_indexing:
               input = [num]
               cho_indexing[chosung] = input
           else:
               cho_indexing[chosung].append(num)
               cho_indexing[chosung].sort(key = lambda x: (-x[1], x[0]))
